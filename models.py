...
class Generator(torch.nn.Module):
    def __init__(self, h):
        ...
        self.down_sample_time = nn.ModuleList()
        for i, (u, k) in enumerate(zip(h.upsample_rates, h.upsample_kernel_sizes)):
            self.down_sample_time.append(weight_norm(
                Conv1d(h.upsample_initial_channel // (2 ** (i+1)), h.upsample_initial_channel // (2 ** (i+1)),
                                k, 2, padding=(k - 2) // 2)))
    ...
    def concat_inputs_by_freq(self, read, pitch, in_inference=False):
    ...
    def concat_inputs_by_time(self, read, pitch, ups_idx, in_inference=False):
        channels = read.size(dim=1)
        time = read.size(dim=2)
        if not in_inference:
            device = torch.device('cuda:{:d}'.format(0))
            ...
            device = None
        result = torch.zeros([self.h.batch_size, channels, time*2], device=device)
        for i in range(time):
            result[:, :, 2 * i + 1] = pitch[:, :, i]
            result[:, :, 2 * i] = read[:, :, i]
        result = self.down_sample_time[ups_idx](result)
        return result
    ...
    def forward(self, x, y_sine_pitch_mel, in_inference=False):
        ...
        for i in range(self.num_upsamples):
            ...
            x = self.concat_inputs_by_time(x, y, i, in_inference)
            # x = (x + y) / 2 #V1
            xs = None
